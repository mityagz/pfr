    ...
    for(std::map<std::string, std::map<int, std::map<int, std::map<int, tparm *>>>>::iterator it0 = r.begin(); it0 != r.end(); it0++) {
       ...
       */
       ...
       if(probe_id == 0) {
         ...
         sql_log.insert(10, dst_ip, probe_id, peer_id, min_rtt, avg_rtt, max_rtt, stddev, mediana, lost, ts);
       } else if(probe_id > 0) {
         if(route.count(dst_ip) == 1 && route[dst_ip].count(probe_id - 1) == 1) {
            if(peer_id == 0) { 
                // case 2.0 no answer from dst
                ...
                sql_log.insert(20, dst_ip, probe_id, peer_id, min_rtt, avg_rtt, max_rtt, stddev, mediana,  lost, ts);
            } else {
                // case 2.1 there is answer from dst
                ...
                sql_log.insert(21, dst_ip, probe_id, peer_id, min_rtt, avg_rtt, max_rtt, stddev, mediana,  lost, ts);
            }
         } else {
            // case 1.1 there isn't prev answer from dst
                ...
                sql_log.insert(11, dst_ip, probe_id, peer_id, min_rtt, avg_rtt, max_rtt, stddev, mediana,  lost, ts);
            }
         }
       min_rtt = 50000; 
       curr_rtt = 0; 
